...
- `--install-service` reports a daemon that fails to start (a port already in use, for example) as a failure and removes the unit or agent, instead of claiming it will start at login.

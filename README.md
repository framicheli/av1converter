...
6. **Encoding and VMAF verification** — Monitor per-file and overall progress; `a` adds more files, folders or a disc while work runs, and `Esc` asks before cancelling. Cancelling during analysis stops only the analysis; files already analysed stay queued. Mean and minimum sampled-frame VMAF scores are computed after each file; the source is deleted only if `delete_source_on_success` is on and both meet the threshold (encode mode only, never when audio was transcoded, Dolby Vision set to be kept was converted to HDR10 by a hardware encoder, a selected subtitle was converted or left out, cover art or attachments are not carried over, the source has extra video or data streams or is Dolby Vision profile 7, or the job was cancelled)

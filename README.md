...
Set `makemkvcon_path` under `[disc]` if it lives somewhere else — a Flatpak install, for instance, needs a small wrapper script since it is run through `flatpak run`. Set `staging_directory` to a scratch drive: a Blu-ray title needs 100 GB or more, and the staging file is deleted once its encode succeeds. Staging directories left behind by an interrupted rip are removed when the TUI or the daemon starts and whenever a disc run ends. An existing output directory must be configured before a rip can start (a disc job cannot write next to its source, which is the staging directory); disc jobs write there even when `same_directory` is on.

...
pub fn t(lang: Language, msg: Msg) -> &'static str {
    ...
    match msg {
        ...
        Msg::WebCfgOutputDirectory => match lang {
            En => "Output directory (also used for disc rips)",
            It => "Cartella di output (usata anche per le estrazioni da disco)",
            Es => "Carpeta de salida (también para las extracciones de disco)",
            Fr => "Dossier de sortie (également pour les extractions de disque)",
            De => "Ausgabeordner (auch für Disc-Rips)",
            Zh => "输出目录（光盘提取也使用）",
        },
